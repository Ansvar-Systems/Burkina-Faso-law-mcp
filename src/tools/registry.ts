/**
 * Tool registry for Burkina Faso Law MCP Server.
...
import { getBurkinabeImplementations, type GetBurkinabeImplementationsInput } from './get-burkinabe-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Burkina Faso Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Burkinabe legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Burkinabe statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (article) from an Burkinabe proclamation or regulation. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Burkinabe legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Burkinabe statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Burkina Faso?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Burkinabe legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Burkinabe proclamation or provision is currently in force, amended, or repealed. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Burkinabe statute aligns with or references. ' +
      'Burkina Faso is not an EU member but several Burkinabe laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Burkinabe statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_burkinabe_implementations',
    description:
      'Find all Burkinabe statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Burkinabe statutes. ' +
      'Note: Burkina Faso implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Burkinabe implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_burkinabe_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Burkinabe aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Burkinabe statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Burkinabe statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_burkinabe_implementations':
          result = await getBurkinabeImplementations(db, args as unknown as GetBurkinabeImplementationsInput);
          break;
