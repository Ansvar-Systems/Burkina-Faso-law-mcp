/**
 * get_burkinabe_implementations — Find Burkinabe statutes implementing a specific EU directive/regulation
...
export interface GetBurkinabeImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface BurkinabeImplementationResult {
  document_id: string;
...
export async function getBurkinabeImplementations(
  db: InstanceType<typeof Database>,
  input: GetBurkinabeImplementationsInput,
): Promise<ToolResponse<BurkinabeImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as BurkinabeImplementationResult[];
