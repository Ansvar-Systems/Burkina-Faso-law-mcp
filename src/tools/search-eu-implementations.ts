/**
 * search_eu_implementations — Search EU directives/regulations that have Burkinabe implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_burkinabe_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  burkinabe_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as burkinabe_statute_count
  ...
  if (input.has_burkinabe_implementation) {
    sql += ' HAVING burkinabe_statute_count > 0';
  }
