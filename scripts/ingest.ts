#!/usr/bin/env tsx
/**
 * Burkina Faso Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawburkinabe.com. Burkinabe proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawburkinabe.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'burkinabe-law-mcp/1.0 (https://github.com/Ansvar-Systems/burkinabe-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Burkina Faso Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawburkinabe.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawburkinabe.com (Federal Negarit Gazette PDFs)`);
