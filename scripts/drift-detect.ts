#!/usr/bin/env tsx
/**
 * Drift detection for Burkina Faso Law MCP.
 *
 * Checks if upstream Burkina Faso Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Burkina Faso Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'burkinabe-law-mcp/1.0 drift-detect' },
      });
