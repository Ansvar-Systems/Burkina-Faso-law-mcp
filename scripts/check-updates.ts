#!/usr/bin/env tsx
/**
 * Burkinabe Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawburkinabe.com';
const PORTAL_NAME = 'Burkina Faso Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/burkinabe-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Burkinabe Law MCP — Data Freshness Check');
